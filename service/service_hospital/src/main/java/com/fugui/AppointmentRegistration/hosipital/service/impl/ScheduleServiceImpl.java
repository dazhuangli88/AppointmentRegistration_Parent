...
import com.fugui.AppointmentRegistration.hosipital.mapper.ScheduleMapper;
...
import com.fugui.AppointmentRegistration.model.hosp.BookingRule;
...
import com.fugui.AppointmentRegistration.vo.hosp.BookingScheduleRuleVo;
...
public class ScheduleServiceImpl extends ServiceImpl<ScheduleMapper, Schedule> implements ScheduleService {
    ...
    public List<Schedule> getDetailSchedule(String hoscode, String depcode, String workDate) {
        ...
        List<Schedule> scheduleList =
                scheduleRepository.findScheduleByHoscodeAndDepcodeAndWorkDate(hoscode,depcode,calendar.getTime());
    ...
    public Map<String, Object> getBookingScheduleRule(Integer page, Integer limit, String hoscode, String depcode) {
        ...
        if(!CollectionUtils.isEmpty(scheduleVoList)) {
            for (BookingScheduleRuleVo scheduleRuleVo : scheduleVoList) {
                String date = dateFormat.format(scheduleRuleVo.getWorkDate());
                if (!scheduleVoMap.containsKey(date)) {
                    scheduleVoMap.put(date,scheduleRuleVo);
                }
            }
        }
        ...
        for(int i=0,len=dateList.size();i<len;i++) {
            ...
            BookingScheduleRuleVo bookingScheduleRuleVo = scheduleVoMap.get(dateFormat.format(date));
            //如果当天没有排班医生
...
//停号时间
    ...
    private IPage getListDate(int page, int limit, BookingRule bookingRule) {
        ...
        for(int i=start;i<end;i++) {
            ...
            calendar.add(Calendar.HOUR_OF_DAY,8);//mogonDB时间比java时间多8小时，固定加八小时去查询
            pageDateList.add(calendar.getTime());
        }
        //如果可以显示数据大于7，进行分页
